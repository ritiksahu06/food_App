...
const Save = () => {
  ...
  useEffect(() => {
  axios
    .get(`${import.meta.env.VITE_BACKEND_URL}/api/food/save`, { withCredentials: true })
    .then(response => {
      const savedFood = response.data.saveFood.map(item => ({
  _id: item._id || item.food?._id,
  video: item.video || item.food?.video,
  description: item.description || item.food?.description,
  likeCount: item.likeCount || item.food?.likeCount,
  saveCount: item.saveCount || item.food?.saveCount,
  isLiked: item.isLiked || false,
}));


      setSaveFood(savedFood);
    })
    .catch((error) => {
